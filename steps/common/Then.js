...
import checkLocalStorageContent from '../../support/check/checkLocalStorageContent';
import checkLocalStorageExists from '../../support/check/checkLocalStorageExists';
...
import checkLinks from "../../support/check/checkLinks";
...
Then(
    /^I expect that (button|element|field value) "([^"]*)?"( not)* matches the text "([^"]*)?"$/,
    checkEqualsText
);
...
Then(
    /^I expect that the( css)* attribute "([^"]*)?" from element "([^"]*)?" is( not)* contains "([^"]*)?"$/,
...
Then(
    /^I expect that local storage "([^"]*)?"( not)* contains "([^"]*)?"$/,
    checkLocalStorageContent
);

Then(
    /^I expect that local storage "([^"]*)?"( not)* exists$/,
...
Then(
    /^I'm waiting for(?: (\d+)ms)*.*$/,
    (ms) => {
        browser.pause(ms||5000);
    }
);
...
Then(
    /^I expect links for element "([^"]*)?":$/,
    checkLinks
);

Then(
    /^I expect domain "([^"]*)" for all links "([^"]*)"$/,
    checkLinksDomain
);
